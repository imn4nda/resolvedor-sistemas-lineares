...
void gaussSeidel(int n, float A[n][n], float b[n], float precisao, float x[n]) {
    float *xAnterior = (float *)malloc(n * sizeof(float));
    if (!xAnterior) {
        fprintf(stderr, "Erro de alocacao de memoria em gaussSeidel\n");
        exit(1);
    }
